...
/**
 * Admin API Route for Branch Tracking Requests
 * 
 * This route uses service_role key to access all branch tracking requests.
 * Only accessible by authenticated admin users.
 */
export async function GET(req: NextRequest) {
  try {
    console.log('Branch tracking requests API called');
    ...
    // Fetch all branch tracking requests
    ...
    if (requestsError) {
      console.error('Error fetching branch tracking requests:', requestsError);
      return NextResponse.json(
        { error: 'Failed to fetch branch tracking requests', details: requestsError.message },
        { status: 500 }
      );
    }

    console.log('Fetched branch tracking requests:', requestsData?.length || 0);
    ...
    console.error('Error in branch tracking requests API:', error);
    return NextResponse.json(
