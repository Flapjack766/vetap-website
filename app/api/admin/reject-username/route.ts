...
import { renderUsernameRejectionEmailHTML } from '@/lib/mail';
  ...
  try {
    ...
    if (userEmail) {
      try {
        ...
        const emailHTML = renderUsernameRejectionEmailHTML({
          name: userName,
          email: userEmail,
        ...
        await resend.emails.send({
          ...
          to: userEmail,
          subject: locale === 'ar' 
            ? `VETAP • طلب اسم المستخدم المخصص - تم الرفض`
            : `VETAP • Custom Username Request - Rejected`,
          html: emailHTML,
        });
      } catch (emailError) {
    ...
    console.error('Error in reject username API:', error);
    return NextResponse.json(
